...
try:
    ...
    with st.sidebar.expander("📖 Passo a Passo & Comandos", expanded=False):
        ...
        **3. Servidores Windows (Não Zabbix)**
        ...
        - Execute o comando unificado abaixo:
        ```powershell
        Write-Output "===HURP_DIAGNOSTICO==="; Write-Output "HOST: $env:COMPUTERNAME"; Write-Output "---DISK---"; Get-Volume | Where-Object {$_.DriveType -eq 'Fixed' -and $_.DriveLetter} | Format-Table DriveLetter, FileSystemLabel, SizeRemaining, Size -HideTableHeaders; Write-Output "---MEM_SWAP---"; Get-CimInstance Win32_OperatingSystem | Select-Object TotalVisibleMemorySize, FreePhysicalMemory, TotalVirtualMemorySize, FreeVirtualMemory | Format-List; Write-Output "---CPU_PERCENT---"; Get-CimInstance Win32_Processor | Measure-Object -Property LoadPercentage -Average | Select-Object -ExpandProperty Average; Write-Output "---TOP_CPU---"; $cores = (Get-CimInstance Win32_ComputerSystem).NumberOfLogicalProcessors; $p1 = Get-Process | Select-Object Id, CPU; Start-Sleep -Milliseconds 2000; $p2 = Get-Process; $cpu = @(); foreach ($p in $p2) { $old = $p1 | Where-Object Id -eq $p.Id; if ($old -and $p.CPU -ne $null -and $old.CPU -ne $null) { $cpu += [PSCustomObject]@{Name=$p.Name; CPU=(($p.CPU - $old.CPU) / 2.0) / $cores * 100; WorkingSet=$p.WorkingSet} } }; $cpu | Sort-Object CPU, WorkingSet -Descending | Select-Object -First 5 Name, @{N='CPU';E={[math]::Round($_.CPU, 2)}}, WorkingSet | Format-Table -HideTableHeaders; Write-Output "---SERVICES---"; Get-Service -Name "IISADMIN", "Tomcat*", "MConnect", "IDCE", "Cache*" -ErrorAction SilentlyContinue | Select-Object Name, Status | Format-Table -HideTableHeaders
        ```
